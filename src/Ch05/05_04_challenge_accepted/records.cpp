...
void StudentRecords::report_card(int _id) {

    std::string sname;

    for(auto& student : students) {

        if(student.get_id() == _id) {
    ...
    for(auto& grade : grades) {
        if(grade.get_student_id() != _id)    continue;

        for(auto& course : courses) {
            if(course.get_id() != grade.get_course_id()) continue;

            std::cout << course.get_name() << ": " << grade.get_grade() << std::endl;
        }
    }

    std::cout << std::endl << std::endl << "GPA: " << get_GPA(_id) << std::endl << std::endl;
